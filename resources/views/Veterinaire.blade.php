...
document.addEventListener('DOMContentLoaded', function() {
    ...
    veterinaireIds.forEach(id => {
        ...
        // Quand tu reçois un message via Pusher (le vétérinaire qui parle)
        channel.bind('chat', function(data) {
            const messageHtml = `
                <div class="message text-start">
                    <div class="p-2 bg-light text-dark d-inline-block rounded mb-2">
                        ${data.message}
                    </div>
                </div>
            `;
            $(`#messages-${id}`).append(messageHtml);
            $(document).scrollTop($(document).height());
        });

        // Quand toi tu envoies un message
            ...
            $.ajax({
                ...
                const messageHtml = `
                    <div class="message text-end">
                        <div class="p-2 bg-success text-white d-inline-block rounded mb-2">
                            ${message}
                        </div>
                    </div>
                `;
