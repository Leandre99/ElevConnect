...
<body>
    ...
    <div class="container">
        ...
        @if ($completedTasksCount > 0)
                ...
                <tbody>
                    @foreach ($completedTasks as $completedTask)
                        <tr>
                            ...
                            <td>{{ $completedTask->user->name ?? 'Utilisateur inconnu' }}</td>
                            <td>{{ $completedTask->tache->nomtache ?? 'Tâche inconnue' }}</td>
                            <td>{{ $completedTask->completed_at ? 'Complétée' : 'Non Complétée' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
            ...
            <p>Tâches Complétées: {{ $completedTasksCount }} / {{ $totalTasks }}</p>
            <p>Date de fin de l'élevage : {{ $endDatePrediction }}</p>
        @else
