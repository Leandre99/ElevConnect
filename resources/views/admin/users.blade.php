                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        @if ($user->role == 'Veterinaire')
                                            <form
                                                action="{{ $user->status == 1 ? route('admin.users.deactivate', $user) : route('admin.users.activate', $user) }}"
                                                method="POST" class="d-inline">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="btn btn-sm {{ $user->status == 1 ? 'btn-success' : 'btn-danger' }}"
                                                    onclick="return confirm('{{ $user->status == 1 ? 'Confirmer la désactivation ?' : "Confirmer l'activation ?" }}')">
                                                    <i
                                                        class="fas {{ $user->status == 1 ? 'fa-toggle-on' : 'fa-toggle-off' }}"></i>
                                                </button>
                                            </form>
                                        @endif
                                    </td>
