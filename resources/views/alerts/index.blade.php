                ...
                <tr class="{{ !$alert->is_active ? 'table-secondary' : '' }}">
                    ...
                    <td>
                        <div class="d-flex flex-wrap gap-2">
                            <button class="btn btn-sm btn-outline-info" title="Voir détails" data-bs-toggle="modal"
                                data-bs-target="#alertModal{{ $alert->id }}">
                                <i class="bi bi-info-circle"></i>
                            </button>

                            <a href="{{ route('completedTasks.soinsParFerme', ['farm' => $alert->ferme->id]) }}"
                                class="btn btn-sm btn-outline-secondary" title="Historique médical">
                            ...
                            <form action="{{ route('alerts.disable', $alert->id) }}" method="POST"
                                ...
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="btn btn-sm btn-outline-danger" title="Désactiver alerte">
                            ...
                            <button class="btn btn-sm btn-outline-primary" title="Planifier réunion"
                                data-bs-toggle="modal" data-bs-target="#planMeetingModal{{ $alert->id }}">
                                <i class="bi bi-calendar-check"></i>
                            </button>

                            <button class="btn btn-sm btn-outline-success" title="Soumettre diagnostic"
                                data-bs-toggle="modal" data-bs-target="#diagnosticModal{{ $alert->id }}">
