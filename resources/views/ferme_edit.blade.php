...
<div class="container">
    <h1>Modifier la ferme : {{ $ferme->nomferme }}</h1>
    ...
    <form method="POST" action="{{ route('ferme.update', $ferme->id) }}">
        @csrf
        @method('PUT')

        <div class="form-group">
            <label for="nomferme">Nom de la ferme</label>
            <input type="text" class="form-control" id="nomferme" name="nomferme" value="{{ $ferme->nomferme }}" required>
        </div>

        <div class="form-group">
            <label for="especes">Espèces</label>
            <select class="form-control" id="especes" name="especes" required>
                <option value="">Sélectionner une espèce</option>
                <option value="bovins" {{ $ferme->especes == 'bovins' ? 'selected' : '' }}>Bovins</option>
                <option value="ovins" {{ $ferme->especes == 'ovins' ? 'selected' : '' }}>Ovins</option>
                <option value="caprins" {{ $ferme->especes == 'caprins' ? 'selected' : '' }}>Caprins</option>
                <option value="porcs" {{ $ferme->especes == 'porcs' ? 'selected' : '' }}>Porcs</option>
                <option value="volailles" {{ $ferme->especes == 'volailles' ? 'selected' : '' }}>Volailles</option>
            </select>
        </div>

        <div class="form-group">
            <label for="race">Race</label>
            <input type="text" class="form-control" id="race" name="race" value="{{ $ferme->race }}" required>
        </div>

        <div class="form-group">
            <label for="agemoyen">Âge moyen (en semaines)</label>
            <input type="number" class="form-control" id="agemoyen" name="agemoyen" value="{{ $ferme->agemoyen }}" required>
        </div>

        <div class="form-group">
            <label for="nombre_animaux">Nombre d'animaux</label>
            <input type="number" class="form-control" id="nombre_animaux" name="nombre_animaux" value="{{ $ferme->nombre_animaux }}" required>
        </div>
